...
public class FirstUnique {
    ...
    public static int solution(int[] arr) {
        ...
        Map<Integer, List<Integer>> ss = new HashMap<>();

        for (int i = 0; i < arr.length; i++) {
            if (!ss.containsKey(arr[i])) {
                List<Integer> indexArrayList = new ArrayList<>();
                ss.put(arr[i], indexArrayList);
            }
            ss.get(arr[i]).add(i);
        }

        List<Integer> lowestIndexArray = new ArrayList<>();
        ss.forEach((key,indexList) -> {
            if (indexList.size() == 1) { //unique
                lowestIndexArray.addAll(indexList);
            }
        });

        if (lowestIndexArray.size() > 0) {
            int index =  lowestIndexArray.stream().mapToInt(n -> n).min().getAsInt();
            return arr[index];
        }
        return -1;
    }
